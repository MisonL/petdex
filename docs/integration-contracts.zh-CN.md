...
- 桌面运行时当前的解码像素缓冲上限为 `16 MiB`；因此 `3072x4576` 的 2x v2 图集虽然可以由素材审计脚本规范化检查，但不属于当前桌面发行版已承诺的运行时输入。
- 当前 `main` 的提交入口校验最小尺寸和整体图集比例；只保持比例但产生分数单元格的图片在客户端可能出现裁切或比例异常。需要发布缩放图集时，应同时保证宽度能整除 8、高度能整除对应行数，并保持 `192:208` 单元格比例。

服务端最终接受规则以当前部署对应的 `src/lib/submissions-validation.ts` 实现为准；本页不复制素材审核分支的机器检查规则。客户端和审计脚本可以更早拒绝风险素材，但不能反向证明这些更严格的规则已经部署到公开提交接口。

审核 contact sheet 仍应检查 v2 的全部 11 行；额外行不能因为客户端只播放 9 个标准状态而被静默丢弃。
