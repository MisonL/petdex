...
当前 `main` 的 `validateSubmission` 只校验 `spriteVersionNumber` 是否省略、为 `1` 或 `2`，尚未将声明值与图集行数绑定；独立的素材质量变更会补齐整数单元格边界、版本与图集匹配及审核 contact sheet 的全行检查。在该变更合并前，不要把这些更严格的规则描述为已部署服务端行为。
