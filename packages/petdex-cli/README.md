...
For the CLI/Desktop boundary, atlas version contract, Codex deep-link gate,
OMP profiles, and SSH remote limitations, see
...
| `manifest unavailable` / network timeout | Both the compact v2 and legacy manifest endpoints failed | Check access to `petdex.dev` and `assets.petdex.dev`, verify the operating system or network proxy configuration, then retry `npx -y petdex install <slug>` |
