...
- Collection commands require login, allow at most 10 personal collections per account and 24 approved pets per collection. Use comma-separated slugs with `--pets`; `--all-approved` fails when more than 24 pets are approved, and deletion requires `--yes`.
