...
`petdex` is the catalog CLI for [Petdex](https://petdex.dev): browse, install, submit, edit, and organize animated pixel-art pets (mascots) for coding agents, from the terminal. It installs pets into `~/.petdex/pets/` and `~/.codex/pets/`, submits new pets to the gallery, and manages personal collections of approved pets. The floating mascot itself ships as the separate **Petdex Desktop app** (petdex.dev/download), which installs agent hooks from its own Settings window and updates itself; this CLI is the catalog client. Reach for it when a user wants to install a pet, publish a pet they created, edit one they own, or group their approved pets.
...
| `petdex collection create` | Create a collection with `--title`, `--desc`, `--pets`, `--cover`, `--external-url`, or `--all-approved` |
| `petdex collection edit <id-or-slug>` | Partially update a personal collection; omitted fields are preserved |
| `petdex collection delete <id-or-slug> --yes` | Delete a personal collection after explicit confirmation |
...
| Group approved pets | `petdex collection create|edit|delete ...` |
...
- Collection commands require login and allow at most 10 personal collections per account. The 24-pet limit bounds growth rather than the stored row, so a collection created before it existed can still be renamed or shrunk, but not grown. Use comma-separated slugs with `--pets`; `--all-approved` on `create` fails when more than 24 pets are approved, and deletion requires `--yes`. `--cover` must name a member of the collection; pass `--cover ""` to clear the cover (an omitted `--cover` leaves it alone).
