...
- Collection commands require login, allow at most 10 personal collections per account and 24 approved pets per collection; the pet limit bounds growth, so an over-limit collection created before it existed can still be renamed or shrunk. Use comma-separated slugs with `--pets`; `--all-approved` on `create` fails when more than 24 pets are approved, and deletion requires `--yes`.
